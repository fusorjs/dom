...
describe('initChild', () => {
  ...
  test.each(allStaticCases)(
    'init single child %p toBe %p',
    (provided: any, expected: any) => {
  ...
  describe('dynamic createUpdater', () => {
    ...
    describe('single child', () => {
      ...
      describe('the same text node', () => {
        ...
        test.each(
          [...allStaticCases, ...allDynamicCases].filter(
            ([p]) =>
              !((typeof p === 'function' ? evaluate(p) : p) instanceof Element),
          ),
